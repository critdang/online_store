...
const createUser = async (req, res, next) => {
  try{
    ...
    let uploadedAvatar = await uploadImg(req.file.path);
...
const changeAvatar = async (req, res, next) => {
  const {id} = req.user;
  let changedAvatar = await uploadImg(req.file.path);
...
const forgotPassword = async (req, res, next) => {
  ...
  try{
    ...
    await helperFn.returnSuccess(req, res, 'An email sent to you, please check the mail to reset password');
  }catch(err){
    console.log(err);
  }
};
...
const resetPassword = async (req, res, next) => {
  ...
  const hashPass = helperFn.hashPassword(password);
  try{
